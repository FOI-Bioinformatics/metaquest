...
def count_metadata(
    ...
    try:
        ...
        # Get genome columns
        genome_columns = [col for col in summary_df.columns 
                         if "GCF" in col or "GCA" in col]
        ...
        for genome_column in genome_columns:
            try:
                ...
                if selected_metadata.empty:
                    logger.warning(
                        f"No metadata found for {genome_column} accessions"
                    )
                    continue
...
def count_metadata_by_category(
    ...
    try:
        ...
        contingency_table = pd.crosstab(
            index=metadata_df[category_column],
            columns=metadata_df[count_column]
        )
