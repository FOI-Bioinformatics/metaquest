...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--version', 
        action='version', 
        version=f'MetaQuest v{__version__}'
    )
    ...
    parser_parse_containment = subparsers.add_parser(
        'parse_containment',
        help='Parse containment data from match files'
    )
    ...
    parser_download_metadata = subparsers.add_parser(
        'download_metadata',
        help='Download metadata for SRA accessions'
    )
    parser_download_metadata.add_argument(
        '--email',
        required=True,
        help='Your email address for NCBI API access'
    )
    parser_download_metadata.add_argument(
        '--matches-folder',
        default='matches',
    ...
    parser_parse_metadata = subparsers.add_parser(
        'parse_metadata',
        help='Parse downloaded metadata files'
    )
    ...
    parser_count_metadata = subparsers.add_parser(
        'count_metadata',
        help='Count metadata values by genome'
    )
    ...
    parser_plot_metadata = subparsers.add_parser(
        ...
        help='Plot metadata counts'
    )
    parser_plot_metadata.add_argument(
        '--file-path',
        required=True,
        help='Path to the metadata counts file'
    )
    parser_plot_metadata.add_argument(
        '--title',
        default=None,
    ...
    parser_assemble_datasets = subparsers.add_parser(
        'assemble_datasets',
        help='Assemble datasets from fastq files'
    )
    parser_assemble_datasets.add_argument(
        '--data-files',
        required=True,
        nargs='+',
        help='List of paths to data files'
    )
    parser_assemble_datasets.add_argument(
        '--output-file',
        required=True,
        help='Path to save the assembled dataset'
    )
