...
ALLOWED_BIOINFORMATICS_TOOLS = {
    'fasterq-dump': {
        'safe_params': {
            '--threads', '--progress', '-O', '--temp', '--split-files',
            '--skip-technical', '--include-technical', '--force', '--gzip'
        },
        'description': 'NCBI SRA data download tool'
    },
    'megahit': {
        'safe_params': {
            '-1', '-2', '-o', '--num-cpu-threads', '--memory', '--min-contig-len',
            '--k-min', '--k-max', '--k-step', '--no-mercy', '--bubble-level'
        },
        'description': 'Illumina assembly tool'
    },
    'flye': {
        'safe_params': {
            '--nano-raw', '--nano-corr', '--nano-hq', '--pacbio-raw', '--pacbio-corr',
            '--pacbio-hifi', '--out-dir', '--genome-size', '--iterations', '--meta',
            '--polish-target', '--threads', '--min-overlap', '--keep-haplotypes'
        },
