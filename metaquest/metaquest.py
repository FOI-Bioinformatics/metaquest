...
def main():
    parser = argparse.ArgumentParser(description='MetaQuest: A toolkit for metagenomic data analysis.')
    ...
    parser_download_test_genome = subparsers.add_parser('download-test-genome',
                                                        help='Download a specific test genome from NCBI.')
    ...
    parser_plot_upset.add_argument('--summary-file', default='SRA-summary.txt',
                                   help='File containing the summary data.')
    parser_plot_upset.add_argument('--upset-plot-file', default='upset_plot.png',
                                   help='File where the UpSet plot will be saved.')
    parser_plot_upset.set_defaults(func=plot_upset)

    # Add a new subparser for the 'plot_heatmap' function
    parser_heatmap = subparsers.add_parser('plot_heatmap',
                                           help='Generate a heatmap from the summary file with a threshold.')
    ...
    parser_heatmap.add_argument('--threshold', type=float, default=0.1,
                                help='Threshold below which values will be set to 0.')
    ...
    parser_download_sra = subparsers.add_parser('download-sra',
                                                help='Downloads SRA data for each unique SRA accession in the .csv files in the matches directory.')
    parser_download_sra.add_argument('--matches-folder', default='matches',
                                     help='Folder containing the matches for which to download SRA data.')
    parser_download_sra.add_argument('--fastq-folder', default='fastq',
                                     help='Folder where the downloaded SRA data will be stored.')
    parser_download_sra.set_defaults(func=download_sra)

    parser_assemble = subparsers.add_parser('assemble',
                                            help='Assembles datasets for each .fastq.gz file in the fastq folder.')
