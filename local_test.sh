...
python -m metaquest.cli.main parse_containment --matches-folder matches \
    --parsed-containment-file parsed_containment.txt \
...
python -m metaquest.cli.main count_metadata --metadata-column Sample_Scientific_Name \
    --threshold 0.9 --output-file metadata_counts.txt
...
python -m metaquest.cli.main plot_containment --file-path parsed_containment.txt \
    --column max_containment --plot-type rank --save-format png
...
if [ -f genomes/GCF_000008985.1.fna ]; then
    ...
    python -m metaquest.cli.main extract_target_reads \
        --parsed-containment parsed_containment.txt --genome-id salmonella_subset \
...
(
    unset METAQUEST_DATA
    export HOME="$STORE_HOME"
    python -m metaquest.cli.main download_sra --accessions-file accessions.txt --dry-run \
        --data-root "$STORE_ROOT" 2>&1 \
