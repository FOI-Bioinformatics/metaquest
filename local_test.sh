...
python -m metaquest.cli.main use_branchwater --branchwater-folder branchwater --matches-folder matches
check matches/salmonella_subset.csv

echo "parse_containment"
python -m metaquest.cli.main parse_containment --matches-folder matches --parsed-containment-file parsed_containment.txt \
...
python -m metaquest.cli.main extract_branchwater_metadata --branchwater-folder branchwater --metadata-folder metadata
...
python -m metaquest.cli.main count_metadata --metadata-column Sample_Scientific_Name --threshold 0.9 --output-file metadata_counts.txt
check metadata_counts.txt

echo "plot_metadata_counts"
python -m metaquest.cli.main plot_metadata_counts --file-path metadata_counts.txt --plot-type bar --save-format png
check metadata_counts_bar.png

echo "plot_containment"
python -m metaquest.cli.main plot_containment --file-path parsed_containment.txt --column max_containment --plot-type rank --save-format png
check parsed_containment_rank_max_containment.png

echo "select_datasets"
python -m metaquest.cli.main select_datasets --threshold 0.95 --output accessions.txt
...
python -m metaquest.cli.main status --init --parsed-containment parsed_containment.txt --accessions-file accessions.txt
check metaquest_registry.json

echo "blacklist"
python -m metaquest.cli.main blacklist --add SRR31320538 --reason "walkthrough example"

echo "status --stage excluded"
python -m metaquest.cli.main status --stage excluded

echo "status"
python -m metaquest.cli.main status --parsed-containment parsed_containment.txt --list-missing

if [ -f genomes/GCF_000008985.1.fna ]; then
    echo "extract_target_reads --dry-run"
    python -m metaquest.cli.main extract_target_reads --parsed-containment parsed_containment.txt --genome-id salmonella_subset \
        --genome-fasta genomes/GCF_000008985.1.fna --threshold 0.95 --dry-run
fi

echo "status --json"
python -m metaquest.cli.main status --json > status.json
python -c "
import json
...
(
    ...
    python -m metaquest.cli.main store_init --data-root "$STORE_ROOT"
)
...
(
    unset METAQUEST_DATA
    export HOME="$STORE_HOME"
    python -m metaquest.cli.main download_sra --accessions-file accessions.txt --dry-run --data-root "$STORE_ROOT" 2>&1 \
...
(
    unset METAQUEST_DATA
    export HOME="$STORE_HOME"
    python -m metaquest.cli.main store_status --json --data-root "$STORE_ROOT" | tee store_status.json
)
grep -q '"projects": 1' store_status.json || { echo "FAIL store_status --json: expected \"projects\": 1"; exit 1; }

echo "select_datasets --top-n 5"
python -m metaquest.cli.main select_datasets --threshold 0.95 --top-n 5 --output accessions_top5.txt
