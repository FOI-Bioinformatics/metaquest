...
class TestSRAInfoCommand:
    ...
    def test_configure_parser_with_optional_args(self):
        ...
        args = parser.parse_args([
            ...
            "--email", "test@example.com",
            "--api-key", "mykey123",
            "--output-report", "custom_report.csv",
            "--bandwidth-mbps", "250.5"
        ])
    ...
    def test_execute_success(self, mock_print, tmp_path):
        ...
        mock_downloader.preview_downloads.return_value = (
            mock_metadata, mock_tech_counts, mock_total_size
        )
        ...
        assert result == 0
        mock_sra_enhanced.EnhancedSRADownloader.assert_called_once_with(
            'test@example.com', 'test_key'
        )
...
class TestSRADownloadEnhancedCommand:
    ...
    def test_configure_parser_all_options(self):
        ...
        args = parser.parse_args([
            ...
            "--force",
            "--temp-folder", "/tmp/sra",
            "--blacklist", "bad1.txt", "bad2.txt",
            "--verify-tools",
            "--report-file", "custom_report.csv"
        ])
...
class TestSRAStatsCommand:
    ...
    def test_configure_parser_with_options(self):
        ...
        args = parser.parse_args([
            "--fastq-folder", "custom_fastq",
            "--output-report", "custom_stats.csv",
    ...
    def test_execute_success(self, mock_print, tmp_path):
        ...
        assert result == 0
        mock_sra_metadata.generate_statistics_report.assert_called_once_with(
            fastq_folder, "stats.csv"
        )
    ...
    def test_execute_folder_not_exists(self, mock_print):
        ...
        command = SRAStatsCommand()
        args = argparse.Namespace(
            fastq_folder="/nonexistent/folder",
            output_report="stats.csv",
            accessions=None
        )
