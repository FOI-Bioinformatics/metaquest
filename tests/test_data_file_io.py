...
class TestFileIoIntegration:
    ...
    def test_json_roundtrip(self, tmp_path):
        ...
        original_data = {
            ...
            "config": {"threads": 4, "timeout": 30.5, "debug": True},
        }
    ...
    def test_file_processing_workflow(self, tmp_path):
        ...
        for i in range(3):
            df = pd.DataFrame({"sample": [f"sample_{j}" for j in range(5)], "value": list(range(i * 5, (i + 1) * 5))})
            write_csv(df, data_dir / f"dataset_{i}.csv", index=False)

        # Process files and collect statistics
        def get_stats(file_path):
            df = read_csv(file_path)
            return {"row_count": len(df), "mean_value": float(df["value"].mean()), "max_value": int(df["value"].max())}
